...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var name = parts[1];
            var gType = parts[2];
            var myBool = bool.Parse(parts[3]);
            
            BaseGradeBook gradeBook = null;
            if (gType.Equals("standard"))
            {
                gradeBook = new StandardGradeBook(name, myBool);
            }
            else if (gType.Equals("ranked"))
            {
                gradeBook = new RankedGradeBook(name, myBool);
            }
            else
            {
                Console.WriteLine("{0} is not a supported type of gradebook, please try again", gType);
                return;
            }
